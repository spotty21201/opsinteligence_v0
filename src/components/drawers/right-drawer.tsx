...
export function RightDrawer({
  ...
  const asset = drawer.mode === 'asset' ? assets.find((a) => a.id === drawer.selectedId) : null;
  const project = drawer.mode === 'project' ? projects.find((p) => p.id === drawer.selectedId) : null;

  const projectLogs = useMemo(() => {
    return logs.filter((log) => {
      if (log.project_id !== project?.id) return false;
  ...
  async function submitDailyLog(e: FormEvent) {
    ...
    await fetch('/api/daily-logs', {
      ...
      body: JSON.stringify({
        ...
        downtime_tags:
          logForm.downtime_tag === 'Other'
            ? [logForm.downtime_other || 'Other']
            : logForm.downtime_tag
              ? [logForm.downtime_tag]
              : [],
  ...
  return (
    <aside className={cn('absolute right-0 top-0 z-30 h-full w-[420px] border-l bg-white transition-transform duration-200', drawer.mode === 'closed' ? 'translate-x-full' : 'translate-x-0')}>
        ...
        <div className="h-[calc(100%-57px)] overflow-y-auto p-4">
          ...
          <Tabs value={assetTab} onValueChange={setAssetTab}>
            ...
            <TabsContent value="overview" className="mt-3 space-y-3 text-sm">
              ...
              <div className="rounded-xl border p-3">Constraints: {asset.capability_profile.constraints.join(', ') || 'None'}</div>
            </TabsContent>
            <TabsContent value="assignments" className="mt-3 text-sm">
              <p className="rounded-xl border p-3 text-slate-600">Use Assign to open Dispatch Planner prefilled with this asset.</p>
            </TabsContent>
        ...
        <div className="h-[calc(100%-57px)] overflow-y-auto p-4">
          ...
          <Tabs value={projectTab} onValueChange={setProjectTab}>
            ...
            <TabsContent value="assignments" className="mt-3">
              <div className="rounded-xl border p-3 text-xs text-slate-600">Open Dispatch Planner with project prefilled.</div>
            </TabsContent>
            <TabsContent value="logs" className="mt-3 space-y-2">
              <form onSubmit={submitDailyLog} className="space-y-2 rounded-xl border p-3 text-xs">
                ...
                <div className="grid grid-cols-2 gap-2">
                  <input type="date" className="h-8 rounded-lg border px-2" value={logForm.date} onChange={(e) => setLogForm((curr) => ({ ...curr, date: e.target.value }))} />
                  ...
                  <input type="number" className="h-8 rounded-lg border px-2" placeholder="Hours" value={logForm.hours_worked} onChange={(e) => setLogForm((curr) => ({ ...curr, hours_worked: Number(e.target.value) }))} />
                  <input type="number" className="h-8 rounded-lg border px-2" placeholder="Progress" value={logForm.progress_value} onChange={(e) => setLogForm((curr) => ({ ...curr, progress_value: Number(e.target.value) }))} />
                </div>
                ...
                {logForm.downtime_tag === 'Other' && (
                  <input className="h-8 w-full rounded-lg border px-2" placeholder="Custom downtime tag" value={logForm.downtime_other} onChange={(e) => setLogForm((curr) => ({ ...curr, downtime_other: e.target.value }))} />
                )}
